...
struct MANGOS_DLL_DECL boss_toc_caster_druidAI: public boss_faction_championAI
{
    ...
    uint32 ChooseDamageSpell()
    {
        if (IsSpellInRange(SPELL_FAERIE_FIRE) && !CurrHostileTarget->HasAura(SPELL_FAERIE_FIRE))
            RETURN_SPELL_IF_COOLED(SPELL_FAERIE_FIRE);
        if (IsSpellInRange(SPELL_INSECT_SWARM) && !CurrHostileTarget->HasAura(SPELL_INSECT_SWARM))
...
struct MANGOS_DLL_DECL boss_toc_shadow_priestAI: public boss_faction_championAI
{
    ...
    uint32 ChooseDamageSpell()
    {
        ...
        if (IsSpellInRange(SPELL_SHADOW_WORD_PAIN) && !CurrHostileTarget->HasAura(SPELL_SHADOW_WORD_PAIN))
            return SPELL_SHADOW_WORD_PAIN;
        if (IsSpellInRange(SPELL_VAMPIRIC_TOUCH) && !CurrHostileTarget->HasAura(SPELL_VAMPIRIC_TOUCH))
...
struct MANGOS_DLL_DECL boss_toc_warlockAI: public boss_faction_championAI
{
    ...
    uint32 ChooseDamageSpell()
    {
        ...
        if (!CurrHostileTarget->HasAura(SPELL_CURSE_OF_EXHAUSTION))
            return SPELL_CURSE_OF_EXHAUSTION;

        if (!CurrHostileTarget->HasAura(SPELL_CORRUPTION))
            return SPELL_CORRUPTION;

        if (!CurrHostileTarget->HasAura(SPELL_UNSTABLE_AFFLICTION))
